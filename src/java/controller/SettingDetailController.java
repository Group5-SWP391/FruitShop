...
public class SettingDetailController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("typeList", setdao.getSettingType());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (id == 0) {
            ...
            if (setdao.checkSetting(type, Integer.parseInt(value)) || !setdao.checkExist(setdao.listType.get(type - 1).getTypeName(), value)) {
                request.setAttribute("er", "The setting already existed or mapping value not exist");
                request.setAttribute("typeList", setdao.getSettingType());
                request.setAttribute("id", id);
                request.setAttribute("set", new Setting(id, null, or, stt == 1, name, "", des));
                request.getRequestDispatcher("SettingDetail.jsp").forward(request, response);
            } else {
                setdao.changeSetting(setdao.listType.get(type -1).getTypeName(), stt==1, value);
                setdao.insertSetting(type, or, stt, name, value, des);
            }
        } else {
            int type = Integer.parseInt(request.getParameter("typetxt"));
            if (setdao.checkSetting(type, Integer.parseInt(value)) || !setdao.checkExist(setdao.listType.get(type - 1).getTypeName(), value)) {
                request.setAttribute("er", "The setting already existed or mapping value not exist");
                doGet(request, response);
            } else {
                String oldval = request.getParameter("oldvalue");
                setdao.changeSetting(setdao.listType.get(type -1).getTypeName(), true, oldval);
                setdao.changeSetting(setdao.listType.get(type -1).getTypeName(), stt==1, value);
